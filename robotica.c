...
{   
    ...
    while (true)
    {   
        // El ultrasónico calcula la distancia 
        ...
        bool infrarrojo_izq = infrarrojo_get(INF_IZQ);
        ...
        {   
            ...
            else
            {
                ...
                // El robot avanza
                else if (infrarrojo_fren == 1 && infrarrojo_der == 0 && infrarrojo_izq == 0)
                {   
                    count = 0;
                    motor_forward(MOT_DER_ADE, MOT_DER_ATR);
                    motor_forward(MOT_IZQ_ADE, MOT_IZQ_ATR);
                ...
                else
                {
                    ...
                    if(absolute_time_diff_us(t0, t1)/1e6 >= 2.0)
                    { 
                        motor_reverse(MOT_DER_ADE, MOT_DER_ATR);
                        motor_reverse(MOT_IZQ_ADE, MOT_IZQ_ATR);
                    }
...
float ultrasonico_get_distance_cm(uint8_t trigger, uint8_t echo)
{
    ...
    // Se calcula anche de pulso y se devuelve la distancia
